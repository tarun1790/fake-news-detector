...
def analyze_blocked_url(db: Session, url: str) -> ScanResult:
    ...
    try:
        ...
        report_text = search_response.text or "[No summary text generated by search grounding]"
        ...
        llm_analysis = json.loads(response.text) if response.text else {}
    ...
    ml_score = 70.0
    llm_score = float(llm_analysis.get("trustScore") or 50)
    ...
    # Extract dynamic title from search summary safely
    explanation_text = llm_analysis.get("explanation") or ""
    title = explanation_text[:80] + "..." if len(explanation_text) > 80 else (explanation_text or "Social Media Post Scan")
    ...
    db_result = ScanResult(
        ...
        trust_score=trust_score,
        verdict=verdict,
        bias_rating=llm_analysis.get("biasRating") or "Moderate",
        clickbait_score=int(llm_analysis.get("clickbaitScore") or 0),
        key_findings=json.dumps(llm_analysis.get("keyFindings") or []),
        claims_analysed=json.dumps(llm_analysis.get("claimsAnalysed") or [])
    )
